...
static struct max8998_platform_data max8998_pdata = {
	...
	.buck1_voltage_set	= { 1325000, 1225000, 1075000, 975000 },
...
{
	gpio_set_value(GPIO_EAR_BIAS_EN, wm8994_mic_bias || jack_mic_bias);
}
...
static int s5ka3dfx_power_on(void)
{
	...
	//Arian-L : seperate vga camera power
		/* VGA_PWR_EN - GPJ2(5) */
		err = gpio_request(GPIO_VGA_PWR_EN, "GPJ2");
		if (err) {
			printk(KERN_ERR "failed to request GPJ2(5) for camera control\n");
	...
	if(HWREV <= 4)	//Arian-K : seperate vga camera power
		gpio_free(GPIO_CAM_IO_EN);
...
static void __init aries_machine_init(void)
{
	...
	HWREV = (HWREV & 0x08) ? (HWREV &= (~0x08)):(HWREV |= 0x08);
	...
	if(HWREV>3)
	{
		...
		s3c_gpio_set_drvstrength(GPIO_PON_RESET_R03, S3C_GPIO_DRVSTR_4X);	
	}
...
*/		
		s3c_gpio_cfgpin(GPIO_FLM_RXD, S3C_GPIO_INPUT);
		s3c_gpio_setpull(GPIO_FLM_RXD, S3C_GPIO_PULL_DOWN);
		s3c_gpio_cfgpin(GPIO_FLM_TXD, S3C_GPIO_INPUT);
		s3c_gpio_setpull(GPIO_FLM_TXD, S3C_GPIO_PULL_DOWN);
		break;
	default:
		break;
	}
