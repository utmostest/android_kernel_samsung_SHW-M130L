...
{
    proximity_config.ctrl = 1;
    proximity_config.xorigin = 16;
    proximity_config.yorigin = 12;	
    proximity_config.xsize = 1;
    proximity_config.ysize = 1;
    proximity_config.reserved = 0;
    proximity_config.blen = 0;
    proximity_config.fxddthr = 255;
    proximity_config.fxddi = 10;
    proximity_config.average = 0;
    proximity_config.mvlullrate = 0;
...
uint8_t write_proximity_config(uint8_t instance, touch_proximity_t23_config_t cfg)
{
    ...
    *(tmp + 11) = (uint8_t) (cfg.mvlullrate & 0x00FF);
    *(tmp + 12) = (uint8_t) (cfg.mvlullrate >> 8);
...
void TSP_forced_release_for_call(void)
{
	...
	if ((qt_initial_ok == 0) || (is_suspending != 0))
		return;
	...
	if (driver_setup == DRIVER_SETUP_OK) {
    ...
    #endif
        {
            ...
            if(quantum_msg[0] < 2  || quantum_msg[0] >= 12) {
				...
				if ((quantum_msg[0] == 0) && (quantum_msg[1] == 0) && (quantum_msg[2] == 0)) {
					check_chip_calibration(1);
				}
...
static DEVICE_ATTR(firmware, S_IRUGO | S_IWUSR, firmware_show, NULL);
...
static DEVICE_ATTR(LED, 0664, NULL, led_control_store);
...
static ssize_t set_proximity_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	printk(KERN_INFO "[TSP] %s : operate nothing\n", __FUNCTION__);

	return 0;
}
static ssize_t set_proximity_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	int cmd_no,config_value = 0;
	char *after;
	...
	if(cmd_no == 0)
	{
		printk("[%s] CMD 0 , proximity_config.ctrl = %d\n", __func__, config_value );
		proximity_config.ctrl = config_value;
	}
	else if(cmd_no == 1)
	{
		printk("[%s] CMD 1 , proximity_config.xorigin = %d\n", __func__, config_value );
    	proximity_config.xorigin = config_value;
	}
	else if(cmd_no == 2)
	{
		printk("[%s] CMD 2 , proximity_config.yorigin = %d\n", __func__, config_value );
		proximity_config.yorigin  = config_value;
	}
	else if(cmd_no == 3)
	{
		printk("[%s] CMD 3 , proximity_config.xsize = %d\n", __func__, config_value );
		proximity_config.xsize = config_value;
	}
	else if(cmd_no == 4)
	{
		printk("[%s] CMD 4 , proximity_config.ysize = %d\n", __func__, config_value );
		proximity_config.ysize  = config_value;
	}
	else if(cmd_no == 5)
	{
		printk("[%s] CMD 5 , proximity_config.reserved = %d\n", __func__, config_value );
    	proximity_config.reserved = config_value;
	}
	else if(cmd_no == 6)
	{
		printk("[%s] CMD 6 , proximity_config.blen = %d\n", __func__, config_value );
		proximity_config.blen  = config_value;
	}
	else if(cmd_no == 7)
	{
		printk("[%s] CMD 7 , proximity_config.fxddthr = %d\n", __func__, config_value );
		proximity_config.fxddthr  = config_value;
	}
	else if(cmd_no == 8)
	{
		printk("[%s] CMD 8 , proximity_config.fxddi = %d\n", __func__, config_value );
		proximity_config.fxddi  = config_value;
	}
	else if(cmd_no == 9)
	{
		printk("[%s] CMD 9 , proximity_config.average = %d\n", __func__, config_value );
		proximity_config.average  = config_value;
	}
	else if(cmd_no == 10)
	{
		printk("[%s] CMD 10 , proximity_config.mvlullrate = %d\n", __func__, config_value );
		proximity_config.mvlullrate  = config_value;
	}
	else if(cmd_no == 11)
	{
		printk("[%s] CMD 10 , proximity_config.mvdthr = %d\n", __func__, config_value );
		proximity_config.mvdthr  = config_value;
	}	
	else 
	{
...
static DEVICE_ATTR(set_proximity, 0664, set_proximity_show, set_proximity_store);
...
int read_all_data(uint8_t dbg_mode)
{
    uint8_t status,mode,i;
    uint8_t read_page,read_point;
	uint16_t max_value = 7000, min_value = 14500;
	...
	for(read_page=0; read_page<4; read_page++)
	{
	    ...
	    {
			read_point = (node *2) + 2;
			read_uint16_t(diagnostic_addr + read_point,&qt_refrence);  

//			printk("qt_refrence %d \n",qt_refrence);
		
			if((qt_refrence!=0) &&((qt_refrence > 14500)||(qt_refrence < 7000)))
			{
				state = 1;
				printk("page %d, node %d \n",read_page,node);
				break;
			}
...
ssize_t set_touchkey_led_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    printk( "[TSP] %s is called.. is_inputmethod=%d\n", __func__, touchkey_led_cnt);

	return sprintf(buf, "%u\n", touchkey_led_cnt);			
}
ssize_t set_touchkey_led_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	...
	printk( "[TSP]HiddenMenu : Touchkey Led Backlight setting value = %s \n", touchkey_led_cnt);		 

	return 1;             

}

static DEVICE_ATTR(set_touchkey_led, 0664, set_touchkey_led_show, set_touchkey_led_store);
...
//	qt602240_init_fail = 0;
	...
	if (device_create_file(ts_dev, &dev_attr_set_touchkey_led) < 0)
		pr_err("Failed to create device file(%s)!\n", dev_attr_set_touchkey_led.attr.name);
