...
static struct s5ka3dfx_regset_table effects[] = {
	...
	S5KA3DFX_REGSET_TABLE_ELEMENT(IMAGE_EFFECT_AQUA,
						s5ka3dfx_effect_aqua),
	S5KA3DFX_REGSET_TABLE_ELEMENT(IMAGE_EFFECT_NEGATIVE,
						s5ka3dfx_effect_negative),

	/* s1_camera [add effect for vga camera] */
	S5KA3DFX_REGSET_TABLE_ELEMENT(IMAGE_EFFECT_RED,
						s5ka3dfx_effect_red),
	S5KA3DFX_REGSET_TABLE_ELEMENT(IMAGE_EFFECT_GREEN,
						s5ka3dfx_effect_green),
};
...
static struct s5ka3dfx_regset_table init_vga_reg[] = {
	S5KA3DFX_REGSET_TABLE_ELEMENT(0, s5ka3dfx_init_vga_reg),
};

static struct s5ka3dfx_regset_table init_vga_vt_reg[] = {
	S5KA3DFX_REGSET_TABLE_ELEMENT(0, s5ka3dfx_init_vga_vt_reg),
};
...
/*
static int s5ka3dfx_i2c_read_multi(struct v4l2_subdev *sd, unsigned char *w_data,
...
static int s5ka3dfx_write_regs(struct v4l2_subdev *sd,
			       struct s5ka3dfx_reg regs[], int size, char* name)
#else
static int s5ka3dfx_write_regs(struct v4l2_subdev *sd,
				   ...
				   int state_fps)
{
	...
	dev_dbg(&client->dev, "%s: state_fps : %d state->vt_mode : %d\n", __func__, state_fps, state->vt_mode);
...
static int s5ka3dfx_init(struct v4l2_subdev *sd, u32 val)
{
	...
	pr_debug("state->framesize_index : %d\n", state->framesize_index);
	...
	if (state->vt_mode == 0) {
		if (state->check_dataline)
			...
			err = s5ka3dfx_write_regset_table(sd, init_vga_reg);
	} else {
		err = s5ka3dfx_write_regset_table(sd, init_vga_vt_reg);	/* S5KA3DFX_PREVIEW_VGA */
	}
