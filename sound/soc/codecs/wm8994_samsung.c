...
int wm8994_ampcontrol_record(int on)
{
	...
	if(on) {
		...
		wm8994->codec_state |= (CAPTURE_ACTIVE);
		audio_ctrl_mic_bias_gpio(codec, 'A');
#if 0 // move to wm8994_record_onboard_mic
		if(wm8994->input_source == RECOGNITION) {
			mdelay(120);
		}
#endif
...
static const char *videocall_path[] = {"OFF", "RCV", "SPK", "HP", "3POLE", "BT",  };
        ...
        struct snd_ctl_elem_value *ucontrol)
{
	...
	DEBUG_LOG("set_videocall_path param %d", path_num);
	...
	switch(path_num)
	{
		...
		default:
			DEBUG_LOG_ERR("The video path[%d] does not exists!", path_num);
			return -ENODEV;
			break;
	}
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	switch(control_data)
	{
        ...
        case CMD_CODEC_IGNORE_SET_RCV_ON : 
            DEBUG_LOG("[WM8994] CMD_CODEC_IGNORE_SET_RCV_ON");
            Ignore_Set_Rcv = 1;
			break;

        case CMD_CODEC_IGNORE_SET_RCV_OFF : 
            DEBUG_LOG("[WM8994] CMD_CODEC_IGNORE_SET_RCV_OFF");
            Ignore_Set_Rcv = 0;
			break;

        case CMD_CODEC_SET_NXP_ON : 
            DEBUG_LOG("[WM8994] CMD_CODEC_SET_NXP_ON");
            Voip_NXP = 1;
			break;

        case CMD_CODEC_SET_NXP_OFF : 
            DEBUG_LOG("[WM8994] CMD_CODEC_SET_NXP_OFF");
            Voip_NXP = 0;
			break;
...
static const struct snd_kcontrol_new wm8994_snd_controls[] = {
    ...
    SOC_ENUM_EXT("Video Call Path", path_control_enum[3],
				wm8994_get_videocall_path, wm8994_set_videocall_path),

	SOC_ENUM_EXT("Capture MIC Path", path_control_enum[4],
				wm8994_get_mic_path, wm8994_set_mic_path),

	SOC_ENUM_EXT("Input Source", path_control_enum[5],
...
static void wm8994_shutdown(struct snd_pcm_substream *substream, struct snd_soc_dai *codec_dai)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_CAPTURE) {
		if ( (wm8994->codec_state & VOIP_CALL_ACTIVE) || 
             (wm8994->codec_state & VIDEO_CALL_ACTIVE) ) {
			DEBUG_LOG_ERR("VoIP/VT Call Activated");
		} else {
			wm8994_disable_rec_path(codec, wm8994->rec_path);
		}
